...
router.post("/", (req, res) => {
  ...
  if (bool > 0) {
    res.json({
      success: false,
      msg: `Invitations existe ! cant send another one`,
    });
...
router.get("/listInacceptedByClass/:id", (req, res) => {
  invitations
    .find({ classOb: req.params.id, status: "loading" })
    .populate("classOb")
    .populate("userOb")
    .then((result) => {
      const users = [];
