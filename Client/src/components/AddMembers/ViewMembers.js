...
export function ViewMembers() {
  ...
  const removeStudentFromList = (idUser) => {
    ...
    dispatch(getSectionsByClass(localStorage.getItem("classURL"))).then(
      (res) => {
        console.log(res);
        for (let i = 0; i < res.payload.length; i++) {
          console.log(res.payload[i]);
          const obj = {
            userOb: idUser,
            idSection: res.payload[i]._id,
          };
          dispatch(DeleteStudentFromSection(obj));
        }
      }
    );
  };
