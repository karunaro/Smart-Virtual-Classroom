...
export const ListInacceptedByClass = createAsyncThunk(
  "invitations/ListInacceptedByClass",
  async (idClass) => {
    const { data } = await axios.get(
      process.env.REACT_APP_BACKEND_PROTOCOL +
        process.env.REACT_APP_BACKEND_IP +
        ":" +
        process.env.REACT_APP_BACKEND_PORT +
        "/invitation/listInacceptedByClass/" +
        idClass
    );
...
export const InvitationsSlice = createSlice({
  ...
  extraReducers: {
    ...
    [ListInacceptedByClass.fulfilled]: (state, action) => {
